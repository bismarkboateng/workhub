...
<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-20">
    {cards.map((card) => (
        <div key={card.id} className="rounded-xl border shadow-sm transform cursor:pointer hover:scale-105 transition-transform duration-200">
          <Image src={card.url} alt={card.title} width={500} height={300} />
          <div className="p-4">
